...
def run_process( scr, input = None, timeout = 2 ):
    ...
    try:
        ...
        print( "{}\t{}\t(*error*) timedout".format(scr[0],fail) )
        return None
...
def check_proc_out( scr, output, input = None, timeout = 2 ):
    ...
    try:
        ...
        if ( output in proc.stdout ):
            ...
            print( "{}\t{}\t(*error*) got {}".format(scr[0],fail, proc.stdout) )
            return False
    except subprocess.TimeoutExpired:
        print( "{}\t{}\t(*error*) timedout".format(scr[0],fail) )
        return False
...
def test_lista_condutores( t ) :
    ...
    # Alínea 1.1
    cleanup( "pessoas.txt" )
    eval_msg( check_proc_out( scr, b'ERRO: O ficheiro "' ), '1.1' )
...
def test_altera_portagem( t ):
    ...
    eval_msg( check_proc_out( [ scr, 'A' ],
         b'ERRO: N\xc3\xa3o foram passados argumentos suficientes!\n' ),
        '2.1.2' )

    eval_msg( check_proc_out( [ scr, 'A', 'B' ],
         b'ERRO: N\xc3\xa3o foram passados argumentos suficientes!\n' ),
        '2.1.3' )

    eval_msg( check_proc_out( [ scr, 'A', 'B', 'C' ],
         b'ERRO: O formato do argumento "' ),
        '2.1.4' )

    eval_msg( check_proc_out( [ scr, 'Leiria-Pombal', 'B', 'C' ],
         b'ERRO: O formato do argumento "' ),
        '2.1.5' )

    eval_msg( check_proc_out( [ scr, 'Leiria-Pombal', 'A2', 'C' ],
         b'ERRO: O formato do argumento "' ),
        '2.1.6' )
    ...
    proc = run_process( [scr, 'Lisboa-Setúbal', 'A2', '5'] )
    if ( proc ):
        ...
        eval_msg( ( b'\n15:Lisboa-Set\xc3\xbabal:A2:5\n' in proc.stdout ), '2.2.2' )

        new = list(portagens)
        new.append("15:Lisboa-Setúbal:A2:5")
        create_file( 'portagens.eval', new )
        eval_msg( compare_files( "portagens.txt", "portagens.eval", verbose = True ), '2.2.3')
        os.remove( 'portagens.eval')

    else :
        eval_msg( False, '2.2.1')
        eval_msg( False, '2.2.2')
        eval_msg( False, '2.2.3')

    # Alínea 2.3 (existe ficheiro, lanço existente)
    cleanup( "portagens.txt" )
    create_file( 'portagens.txt', portagens )

    proc = run_process( [scr, 'Lisboa-Porto', 'A1', '15'] )
    if ( proc ):
        ...
        for i in range( len(new) ):
            if ( 'Lisboa-Porto' in new[i] ):
                ...
                break
        create_file( 'portagens.eval', new )
        eval_msg( compare_files( "portagens.txt", "portagens.eval", verbose = True ), '2.3.3')
        os.remove( 'portagens.eval')

    else :
        eval_msg( False, '2.3.1')
        eval_msg( False, '2.3.2')
        eval_msg( False, '2.3.3')
    ...
    if ( proc ):
        ...
        eval_msg( ( b'\n1:Lisboa-Porto:A1:15\n' in proc.stdout ), '2.4.2' )

        new = ['1:Lisboa-Porto:A1:15']
        create_file( 'portagens.eval', new )
        eval_msg( compare_files( "portagens.txt", "portagens.eval", verbose = True ), '2.4.3')
        os.remove( 'portagens.eval')
    else :
        eval_msg( False, '2.4.1')
        eval_msg( False, '2.4.2')
        eval_msg( False, '2.3.3')
...
def test_faturacao( t ):
    ...
    if ( proc ):
        eval_msg( ( b'SUCESSO: Lista de Faturas:\n'
            in proc.stdout ), '3.2.1' )

        create_file_faturas( utilizacao, pessoas, 'faturas.eval' )
        eval_msg( compare_files( "faturas.txt", "faturas.eval", verbose = True ), '3.2.2')
        os.remove( "faturas.eval" )
    else:
        eval_msg( False, '3.2.1')
        eval_msg( False, '3.2.2')
...
def test_stats( t ):
    ...
    #4.1.2 - Falha com argumento inválido
    eval_msg( check_proc_out( [ scr, "bad" ],
         b'ERRO: O formato do argumento "' ),
        '4.1.2' )
    ...
    if ( proc ):
        ...
        eval_msg( res == eval, '4.2.2')
    else:
        eval_msg( False, '4.2.1')
        eval_msg( False, '4.2.2')
    
    cleanup( "portagens.txt")

    #4.3.1 - Falha com registos sem argumentos
    eval_msg( check_proc_out( [ scr, "registos" ],
         b'ERRO: N\xc3\xa3o foram passados argumentos suficientes!\n' ),
        '4.3.1' )
    ...
    proc = run_process( [scr, 'registos', str(nr_registos)] )
    if ( proc ):
        res = proc.stdout.split(b'\n')
        if ( b'SUCESSO: Lista de Nomes:\n' in proc.stdout ):
            eval_msg( True, '4.3.2.a' )
            res.pop(0)
        else:
            eval_msg( False, '4.3.2.a' )
        ...
        eval_msg( success, '4.3.2.b')


    else:
        eval_msg( False, '4.3.2.a')
        eval_msg( False, '4.3.2.b')
    ...
    proc = run_process( [scr, 'condutores'] )
    if ( proc ):
        res = proc.stdout.split(b'\n')
        if ( b'SUCESSO: Lista de Nomes:\n' in proc.stdout ):
            eval_msg( True, '4.4.1' )
        ...
        eval_msg( success, '4.4.2')
    else:
        eval_msg( False, '4.4.1')
        eval_msg( False, '4.4.2')
